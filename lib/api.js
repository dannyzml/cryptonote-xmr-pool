...
var redisCommands = [
    ...
    ['hlen', config.coin + ':workerStats:all']
];
...
function handleWorkerStats(urlParts, response){
    ...
    if (urlParts.query.longpoll === 'true'){
        redisClient.exists(config.coin + ':workerStats:' + address, function(error, result){
            if (!result){
                response.end(JSON.stringify({error: 'not found'}));
                return;
            }
        ...
        redisClient.hgetall(config.coin + ':workerStats:' + address, function(error, result){
            if (error){
                response.end(JSON.stringify({error: 'not found'}));
                return;
            }
  ...
  var server2 = https.createServer(options, function(request, response){
      ...
      switch(urlParts.pathname){
          ...
          default:
              ...
              response.end('Invalid API call:'+urlParts.pathname);
              break;
      }
  });
}
