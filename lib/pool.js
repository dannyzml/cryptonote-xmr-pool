...
function Miner(id, login, pass, ip, startingDiff, noRetarget, pushMessage, workerId){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
Miner.prototype = {
    retarget: function(now){
        ...
        if(newDiff >= options.maxDiff){
            var low = Math.random() * (0.5 - 0.1) + 0.1;
            newDiff = Math.ceil(this.difficulty * (1 - low));
        }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['hset', config.coin + ':workerStats:all', miner.login +'.'+miner.workerId, dateNowSeconds]
        // ['hset', config.coin + ':workerStats:' + miner.login, miner.workerId, [job.difficulty, dateNowSeconds].join(':')]
    ];
    ...
    redisClient.hget(config.coin + ':workerStats:' + miner.login, miner.workerId, function(err, result){
        if (err){
            log('error', logSystem, 'Failed find workerstats %s%s \n %j', [miner.login, miner.workerId, err]);
        }

        if(result){
            var arr = result.split(":");
            var total = parseInt(job.difficulty) + parseInt(arr[3]);
            var lastTime = arr[2];
            if(dateNowSeconds > lastTime){
                redisClient.hset(config.coin + ':workerStats:' + miner.login, miner.workerId, [job.difficulty,lastTime,dateNowSeconds,total].join(':'));
            }
        }else{
            redisClient.hset(config.coin + ':workerStats:' + miner.login, miner.workerId, [job.difficulty,dateNowSeconds,dateNowSeconds,parseInt(job.difficulty)].join(':'));
        }
    ...
    switch(method){
        case 'login':
            ...
            if(config.poolServer.fixedDiff.enabled) {
                ...
                var workerCharPos = login.indexOf(config.poolServer.fixedDiff.workerSeparator);
                if(workerCharPos != -1){
                    workerId = login.substr(workerCharPos + 1);
                    login = login.substr(0, workerCharPos);
                    log('info', logSystem, 'Miner Name is %s',  [workerId]);
                }
            }
            ...
            miner = new Miner(minerId, login, params.pass, ip, difficulty, noRetarget, pushMessage, workerId);
	    ...
	    params.nonce = params.nonce.substr(0, 8).toLowerCase();
            ...
            if (!blockTemplate){
                var minerText = miner ? (' ' + miner.login + '+' + miner.workerId + '@' + miner.ip) : '';
